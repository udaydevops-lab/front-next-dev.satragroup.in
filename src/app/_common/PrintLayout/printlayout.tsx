...
const Printlayout = (props: any) => {
  ...
  const getPatientData = async () => {
    ...
    let patientData = patientid ? patientid : props.patientid;
    let opdEncounterIdData = opdEncounterId
      ? opdEncounterId
      : props.opdEncounterId;
    const data = await services.get(
      getPatientDetailsforPrint + patientData + "/" + opdEncounterIdData
    );
    const deportmentData: any = await services.get(getAllDepartments);
    const res = data.data
    const lastObject = res[res.length - 1];
    console.log(lastObject)
    setPatientData(lastObject);
  };
    ...
    <>
      ...
      <div
        style={{
          width: "100%",
          float: "right",
          marginTop: `${props.verifiedBy ? "5px" : "100px"}`,
          textAlign: "right",
        }}
      >
        <span> {props.verifiedBy ? props.verifiedBy : patientData?.doctor}</span>
        {/* {patientData?.doctor} */}
      </div>
