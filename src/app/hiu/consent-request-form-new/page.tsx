...
export default function ConsentRequestFormNew({
  getConsentResult,
  formData,
  type,
  setModaloc,
  ...
  const [healthInfoTo, setHealthInfoTo] = useState<any>(
    moment(formData?.updatedDateRangeTo)
  );
  const [healthInfoFrom, setHealthInfoFrom] = useState<any>(
    moment(formData?.updatedDateRangeFrom)
  );
  const [consentExpiry, setConsentExpiry] = useState<any>(
    moment(formData?.updatedDateErraiseAt)
  );
  ...
  const onRequestConsent = () => {
    if (!purposeOfRequest.label) {
      toast.error("Please select purpose of request");
      return;
    }
    // if (!patientName) {
    //   toast.error("Please confirm patient name");
    //   return;
    // }
    let postObj = {
      purposeOfReq: purposeOfRequest.label,
      healthAddress: patIdentifier + `@${loginResponse?.abhaSuffix=='abdm'?'abdm':'sbx'}`,
    ...
    setLoading(true);
    services
      .create(createConsentNew, postObj)
      .then((response) => {
        setLoading(false);
        setModaloc(false);
      ...
      .catch((error) => {
        ...
        } else if (error.response.data[0]?.error?.message) {
          toast.error(
            `${error.response.data[0]?.error?.code} ${error.response.data[0]?.error?.message}`
          );
      ...
      <div className=" rounded-lg bg-white  shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5">
        ...
        <div className="w-full flex gap-4 mt-4">
          {/* <div className="flex gap-6 w-1/3 items-center ">
            ...
            <div className="flex relative gap-2 input-width w-2/3 ps-4">
        ...
        <div className="flex gap-3 w-1/2 items-center mt-6   ">
          <div className="flex items-star justify-start w-1/3 ">
            Consent expiry <span className="text-red-400">*</span>
          </div>
          <div className="w-2/3 pe-2">
            <DateInput
              enableTime={true}
              value={consentExpiry}
              onChange={(e: any) => {
