...
import { PATIENT_REGISTRATION_ROUTE } from "../utilities/constants";
import {
  Button,
  Dialog,
  DialogBody,
...
export default function AbhaProfile(props: any) {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const handlePatientIdCard = () => {
    ...
    setLoading(true);
    services
      .create(getPatientIdCardByAadhaarMobile, postObj)
      .then((response) => {
        setLoading(false);
        ...
        // );
        setAbhaCardData(response.data.doc);
  ...
  useEffect(() => {
    ...
    services
      ...
      .then((response) => {
        ...
        services
          .get(getPatientProfileImage + response.data.healthIdNumber)
          .then((response: any) => {
            let completeResp = {
              ...profileDataResp,
    ...
    services
      ...
      .then((response) => {
        setLoading(false);
        setQRData("data:image/png;base64," + response.data.image);
      })
      .catch((err) => {
        setLoading(false);
        console.log(err.message);
  ...
  useEffect(() => {
    const updateAbhaAddressByMrnFun = () => {
      ...
      let postMrnObj = {
        mrn: patientMrn,
        abhaAddress: profileData.healthId,
      };
      if (profileData.healthId && patientMrn) {
        services
            ...
            <div className="mt-4 grid grid-cols-12 gap-4 md:gap-6  2xl:gap-7.5">
              ...
              <div className="col-span-12 bg-blue px-5  dark:bg-boxdark sm:px-7.5  xl:col-span-4">
                ...
                <div className="font-sans flex gap-4  py-3">
                  <span>{profileData.healthId}</span>
        ...
        <Box sx={{ width: "100%" }}>
          <Box sx={{ borderBottom: 1, borderColor: "divider" }}>
            ...
            >
              ...
              <Tab onClick={()=>handlePatientIdCard()} className="font-bold" label="ABHA Card" {...a11yProps(2)} />
              <Tab
                className="font-bold"
                label="Delete ABHA"
                {...a11yProps(3)}
                    ...
                    <div style={{ cursor: "pointer" }}>
                      ...
                      <div className="w-[600px] h-[500px]">
                        {abhaCardData ? (
                          <div>
                            <object
                              data={`data:application/pdf;base64,${abhaCardData}`}
