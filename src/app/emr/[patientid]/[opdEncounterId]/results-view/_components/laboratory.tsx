...
import LaboratoryView from "@/app/results/_components/laboratoryView";
...
const [key1,setKey1]=useState(0)
  ...
  const getOrders = async () => {
    try {
      ...
      if(patientName&&mrn){
        const filteredData = responseMain.filter((item: any) => item.patientName).filter((list: any) => list?.patientName.toLowerCase().includes(patientName?.toLowerCase()));
...
const getPatientDetails = async() => {
  ...
  return (
    <>
      <div key={key1} className="grid grid-cols-4 gap-4 mb-5">
