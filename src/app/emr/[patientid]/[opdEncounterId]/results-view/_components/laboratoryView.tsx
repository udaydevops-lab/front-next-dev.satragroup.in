    ...
    const getServiceData = async () => {
        ...
        try {
            const response = await services.get(
                getResultEntry +
                `patientId=${props.modaloc.rowData.patientId}&opdEncouterId=${props.modaloc.rowData.opdEncounterId}&resultEntryType=Laboratory&cpoeId=${props.modaloc.rowData.id}`
            );
        ...
        <>
            ...
            <div className='w-full overflow-auto mt-2'>
                ...
                <div className="w-full flex mt-2">
                    ...
                    <div className='w-3/4 flex justify-end gap-4'>{["OK", "CANCEL", "PRINT"].map((text, index) => <ActionButton key={index} buttonText={text} disabled={text === "PRINT" ? printBtn : null} handleSubmit={index === 0 ? handelCancel : (index === 1 ? handelCancel : PrintRecord)} width="w-[120px] py-3" />)}
                    </div>
                </div>
            </div>
        </>
