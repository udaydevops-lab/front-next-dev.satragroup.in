...
import ProceduresEntry from "./procedureEntrty";
    ...
    try {
      ...
      const response = await services.get(
        ...
        `fromDate=${moment(fromDate).format("YYYY-MM-DD")}&toDate=${moment(
          toDate
        ).format("YYYY-MM-DD")}&cpoeType=Procedures${OrdersType}`
      );
      ...
      <div className={`${props?.screenData?.View === 1 ? "" : "pointer-events-none"}`}>
        <div className="grid grid-cols-4 gap-4 mb-5">
          <div className="flex items-center w-full">
            ...
            />
            <label htmlFor="allOrdersp" className="text-sm cursor-pointer">
              All Orders
            </label>
          </div>
          <div className="flex items-center w-full">
            <input
              ...
              className="border-0 w-6 h-6 mr-2 focus:ring-black"
            />
            <label htmlFor="verifiedOrdersp" className="text-sm cursor-pointer">
        ...
        >
          <DialogHeader className="w-full flex justify-beetween pb-2">
            <div className="w-full text-[20px] text-blue-500">Procedure View</div>
            <div className="text-right">
              <XMarkIcon
                className="cursor-pointer w-5 h-5 text-gray-600 hover:text-gray-900"
          ...
          <DialogBody className=" text-left text-black text-[15px] justify-center pt-0">
            <ProcedureView setModaloc={setModaloc} modaloc={modaloc} />
          </DialogBody>
        </Dialog>
      </div>
    </div>
...
export default roleInfoScreenData(Procedures, "Re");