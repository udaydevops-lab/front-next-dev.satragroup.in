...
import { deleteRadiologyAssignParameter, getAllRadiologyParameters, getRadiologyServiceByServiceType, saveRadiologyAssignParameter } from '@/app/utilities/api-urls'
import { toast } from 'react-toastify'
import Select from 'react-tailwindcss-select'

interface RadiologyAssignTestParameterFormprops {
    fields: any,
    setFields: any,
    parameterGrid: any,
...
const RadiologyAssignTestParameterForm: FC<RadiologyAssignTestParameterFormprops> = ({ fields, setFields, parameterGrid, setParameterGrid, handleAdd, loder, handleSave, handleClear, btnStatus }) => {
    ...
    const deleteRadiologyAssignParameterFun = (row: any) => {
        console.log(row)
        try {
            let itemId = row.assignRadiologyParameterItemId
            let headerId = fields.serviceName.assignRadiologyParameterHeaderId
            const emptyObj = {}
            services.create(`${deleteRadiologyAssignParameter}assignRadiologyParameterHeaderId=${headerId}&assignRadiologyParameterItemId=${itemId}`, emptyObj).then((res) => {
            ...
            renderCell: (params: any) => (
                <div className='flex gap-4 items-center w-[80px]'>
                    <div className='flex justify-center'>
                        {params.row.assignRadiologyParameterItemId && params.row.assignRadiologyParameterItemId ?
                            <TrashIcon
    ...
    const getServiceNameList = async () => {
        try {
            const res = await services.get(getRadiologyServiceByServiceType)
    ...
    const getserviceParameterData = async () => {
        try {
            const res = await services.get(getAllRadiologyParameters)
            const finalValue: any = res.data.map((list: any) => ({
                ...list,
                label: list.radiologyParameterDescription,
                value: list.radiologyParameterDescription,
            }))
...
export default RadiologyAssignTestParameterForm