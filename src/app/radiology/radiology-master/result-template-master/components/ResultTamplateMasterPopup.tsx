...
import { assignRadiologyParameterHeaderId, getAllLabAssignServicesData, getAllLabAssignServicesNames, getAllRadiologyAssignServices, getAllRadiologyParameters, resultTamplateSave, saveRadiologyResultTemplate } from '@/app/utilities/api-urls';
...
const ResultTemplateMasterPopup: FC<ResultTemplateMasterPopupProps> = ({
  ...
  const handelLabServicefeilds = async (row: any) => {
    const id = row.assignRadiologyParameterHeaderId;
    try {
      const response = await services.get(`${assignRadiologyParameterHeaderId}${id}`);

      let result = await response.data.assignParameterItemSet.map((mainList: any, index: any) => {
  ...
  const handelSave = () => {
    ...
    const assignParameterItemSetData = feildsData.map((list: any) => ({
      ...list,
      resultTemplateItemId: btnType === "save" ? null : list.resultTemplateItemId,
    ...
    const postObj = {
      radiologyResultTemplateHeaderId: btnType === "save" ? null : radServiceFeilsInfo.radiologyResultTemplateHeaderId,
      serviceCode: radServiceFeilsInfo.serviceCode,
      serviceDesc: radServiceFeilsInfo.serviceDesc,
      radiologyResultTemplateName: radfeilds.resultTemplateName,
      ...
      radiologyResultTemplateItemDto: assignParameterItemSetData,
    };
    const headers = getHeaderResponse();
    services.create(saveRadiologyResultTemplate, postObj, headers).then((res: any) => {
