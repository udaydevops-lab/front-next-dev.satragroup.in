...
import TextArea from "@/app/_common/textArea";
...
import {
  ...
  getRadiologists,
  getTechnicians,
  saveRadiologyOrderDetails,
  updateRadOrderDetails,
...
function OrderForm(props: any) {
    ...
    services
      ...
      .then((response) => {
        ...
        setFormData({
          ...
          specialityDesc: data.specialityDesc,
          modality: "",
  ...
  const handleInterpretation = () => {
    router.push(`/radiology/results/${orderId}/`);
  };
  const getOrderDetails = () => {
    services
      .get(getOrderDetailsByOrderId + orderId)
      .then((response) => {
        response.data.radiologystName = {
          label: response.data.radiologystName,
          value: "",
        };
        ...
        setType("");
        setArrival("");
        if (!response.data.isPerformedTechRadiologyst) {
          setPerformedBy("technician");
          setKey1((k) => k + 1);
        } else {
          setPerformedBy("radiologist");
          setKey1((k) => k + 1);
        }
  ...
  const onEventClick = (event: string, dateTime: any) => {
    ...
    let postObj = {
      ...formData,
      radiologystName: formData.radiologystName.label,
    ...
    services
      .create(saveRadiologyOrderDetails, postObj)
      .then((response) => {
  ...
  const onUpdate = () => {
    let postObj = {
      orderDetailsId: formData.orderDetailsId,
      isPerformedTechRadiologyst: performedBy == "radiologist" ? 1 : 0,
      technicianName: formData.technicianName.label,
      radiologystName: formData.radiologystName.label,
    ...
    services
      .create(updateRadOrderDetails, postObj)
      .then((response) => {
  ...
  useEffect(() => {
    getList(getRadiologists, setRadiologistsList);
    getList(getTechnicians, setTechnicianList);
    getOrderDetails();
  }, []);
  return (
            ...
            <div className=" flex gap-10" key={key1}>
              <div className="">
                <Radio
                  ...
                  onChange={(e) => {
                    setFormData({
                      ...formData,
                      radiologystName: { label: null, value: null },
                    });
                    setType("type");
              ...
              <div className="">
                <Radio
                  crossOrigin={undefined}
                  value={"radiologist"}
                  defaultChecked={performedBy == "radiologist" ? true : false}
                  onChange={(e) => {
                    setFormData({
                      ...formData,
              ...
              {performedBy == "technician" ? (
                ...
                <div className="w-full my-select-options">
                  <ReactSelectBox
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, radiologystName: e })
                    }
                    options={radiologistsList}
                    value={formData.radiologystName}
                  />
                </div>
              )}
            </div>
