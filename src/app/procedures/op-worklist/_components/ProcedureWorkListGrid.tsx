...
import { getProcedureMoreDetails } from "@/app/utilities/api-urls";
import { toast } from "react-toastify";
import { ProcedureGridProps } from "./ProcedureWorkListInterfaces";
import { getContent } from "./Utils";

function ProcedureGridField({ data }: ProcedureGridProps) {
  ...
  const handleAcc = (data: any) => {
    router.push(`/procedures/${0}/${data.orderId}/order-details`); //should pass patId and encounter Id
    setFormData(data);
    handleOpen();
  };
  ...
  const getBillDetails = (data: any) => {
    services
      .get(getProcedureMoreDetails + data.orderId + "/" + data.status)
      .then((response) => {
        ...
        if (response.data.allProceduresOrderDataByParentId.length > 0) {
          let recordedBy =
            response.data.allProceduresOrderDataByParentId[0].recordBy;
          obj.recordedBy = recordedBy;
          obj.recordedDateTime = moment(
            response.data.allProceduresOrderDataByParentId[0].recordedDateTime
...
export default ProcedureGridField;
