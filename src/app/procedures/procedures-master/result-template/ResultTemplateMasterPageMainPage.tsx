...
import { getAllRadiologyAssignServices, getProcedureResultTemplate, getProcedureResulUpdateById, getRadiologyResultTamplate, getRadiologyResulUpdateById, getResultTamplate, proceduresServiceName } from '@/app/utilities/api-urls'
...
const ResultTemplateMasterPageMainPage = () => {
    ...
    const [radfeilds, setRadFeilds] = useState<any>({
        radServiceName: { label: "Procedures Service Name" },
        //labServiceFeilsInfo: {},
    })
    ...
    const getgridData = () => {
        //getResultTamplate
        services
            .get(`${getProcedureResultTemplate}`)
            .then((response) => {
    ...
    const getRadServiceNamelist = async () => {
        // radServiceNameData
        try {
            const response = await services.get(`${proceduresServiceName}`);
            console.log(response.data)
    ...
    const handleEdit = (row: any) => {
        let id = row.proceduresResultTemplateHeaderId
        console.log(row)
        services
            .get(`${getProcedureResulUpdateById}${id}`)
            .then((response) => {

                console.log(response.data)
                let result = response.data.proceduresResultTemplateItemDto.map((mainList: any, index: any) => {
                    if (mainList.resultType === "list") {
                        ...
                        if (mainList.valueList === null || mainList.valueList === "") {
                            return {
                                ...mainList,
                                procedureParameterDesc: mainList.parameterDesc,
                                options: []
                            };
                        } else {
                    ...
                    mainList.procedureParameterDesc = mainList.parameterDesc;
                    return mainList;
                });
                setFeildsData(result)

                let Mainresult = response.data.proceduresResultTemplateItemDto.map((list: any) => ({
                    ...list,
                ...
                setRadFeilds({
                    ...
                    resultTemplateName: response.data.proceduresResultTemplateName
                })
