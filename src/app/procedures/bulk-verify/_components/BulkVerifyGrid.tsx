...
import {
  procedureResultEntryBulkVerify,
...
const BulkVerifyGrid: FC<BulkVerifyGridProps> = ({
      ...
      renderCell: (params: any) => (
        <div
          ...
          onClick={() =>
            ...
            // )
            router.push(`/procedures/results/${params.row.orderId}/`)
          }
        >
          {params.row.orderId}
        </div>
  ...
  // row id
  const getRowId = (row: any) => row.procedureEnryHeaderId;
  ...
  const handelVerify = () => {
    if (selectedRows?.length === 0) {
      ...
      const postObj = selectedRows.map((item: any) => ({
        ...
        procedureEnryHeaderId: item.procedureEnryHeaderId,
        verifiedBy: verifyedByVal.label,
      }));
      services
        .create(procedureResultEntryBulkVerify, postObj, headers)
        .then((res) => {
