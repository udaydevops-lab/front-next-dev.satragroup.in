...
import ProcedurePrintLayout from '@/app/_common/PrintLayout/ProcedurePrintLayout';
...
import { getAllEquipmentData, getAllProcedureDoctors, getDepartmentPrac, getLabEquipment, getProcedureEquipment } from '@/app/utilities/api-urls';
...
const ResultEnteryForm: FC<ResultEnteryFormProps> = ({ feildsData, setFeildsData, listSel, setListSel, selectfeilds, setSelectFeilds, btnStatues, loader, onSave, setLoader, setConsumables, consumables, onVerify, verified, loader1, patientData, orderData, screenInfo }) => {
    ...
    const handleReset = () => {
        setListSel({})
        setFeildsData((prevData: any) =>
            prevData.map((item: any, i: number) => { return { ...item, result: "" } })
        );
    }
    ...
    // const getEquipment = () => {
    //     services.get(getProcedureEquipment).then((response) => {
    //        // const res: any = filterByDepartmentCode(response.data, patientData?.department)
    //         let result: any = res.map((list: any) => ({
    //             ...list,
    ...
    const getEquipment = () => {
        const url: any = `${getAllEquipmentData}specialityCode=&departMentCode=${patientData?.department}&type=ST1010`
    ...
    // const filterByDepartmentCode = (data: any, departmentCode: any) => {
    ...
    //         .filter((equipment: any) => equipment.departmentCode === departmentCode);
    //     return res
    // };
            ...
            <div className='w-full mt-6 pb-4 flex gap-3 justify-end'>
                ...
                <div className='flex gap-4'>
                    {/* <ActionButton
                            ...
                            // buttonText={btnStatues === "save" ? "SAVE" : "UPDATE"}
