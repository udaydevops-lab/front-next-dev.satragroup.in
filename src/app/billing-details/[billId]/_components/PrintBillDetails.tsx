...
import { getContent, getPrintContent } from './Utils'
import moment from 'moment';
import services from '@/app/utilities/services';
import { gethospitalDetials, getPatientDetailsforPrint } from '@/app/utilities/api-urls';
import { getLocalItem } from '@/app/utilities/local';

export default function PrintBillDetails(props:any) {
 
    const [patientData, setPatientData] = useState<any>();
    const [addressDetails, setAddressDetails] = useState<any>({});
    const getPatientData = async () => {
        const res = await services.get(
            getPatientDetailsforPrint + props.patientid + "/" + props.opdEncounterId
        );
        let result = res.data.filter((list: any) => list.billNumber === props.billNumber)
        const lastObject = result[result.length - 1];
        let final = result.length > 1 ? lastObject : result
        setPatientData(final);
    };
    ...
    useEffect(() => {
        getPatientData();
        getHsopitalDetails();
    }, [props.patientid, props.opdEncounterId]);
    ...
    return (
        <>
            ...
            <div style={{ width: "30%", marginRight: "5%", float: "left",marginBottom: "5px", }}>
                ...
                >
                    ...
                    >
                        <div style={{ width: "30%", marginRight: "5%", float: "left" }}>
                            <span style={{ width: "auto", float: "left" }}>
                                <b>Enc No: </b>
                            </span>
                            <span style={{ width: "auto", float: "left" }}>
                        ...
                        <div style={{ width: "65%", float: "left", display: "flex" }}>
                            <span style={{ width: "auto", float: "left" }}>
                                <b>Patient Name: </b>
                            </span>
                            <span style={{ width: "auto", float: "left" }}>
        ...
        <table style={{width:"100%",tableLayout:'fixed',textAlign:'left'}}>
          <thead >
            <tr >
              <th >S.No.</th>
              <th style={{paddingLeft:'20px',padding:'2px'}} >Service Name</th>
              <th style={{paddingLeft:'20px',padding:'2px'}} >Service Type</th>
              <th style={{paddingLeft:'20px',padding:'2px'}} >Bill Amount</th>
              <th style={{paddingLeft:'20px',padding:'2px'}} >Discount</th>
              <th style={{paddingLeft:'20px',padding:'2px'}} >Net Amount</th>
              <th style={{paddingLeft:'20px',padding:'2px'}} >Status</th>
            </tr>
          </thead>
          <tbody >
            {props.tableData.map((item:any, index:number) => (
              <tr  className='border-collapse border-black   text-sm ps-10' key={index}>
                <td>{index + 1}</td>
                <td style={{paddingLeft:'20px',padding:'2px'}} >{item.serviceName}</td>
                <td style={{paddingLeft:'20px',padding:'2px'}} >{item.serviceTypeDesc}</td>
                <td style={{paddingLeft:'20px',padding:'2px'}} >{item.serviceAmount}</td>
                <td style={{paddingLeft:'20px',padding:'2px'}} >{item.discountAmount}</td>
                <td style={{paddingLeft:'20px',padding:'2px'}} >{item.finalAmount}</td>
                <td style={{paddingLeft:'20px',padding:'2px'}} >{item.status}</td>
              </tr>
            ))}
          </tbody>
        </table>
     ...
     {getPrintContent(props.totalData)}
    </div>
